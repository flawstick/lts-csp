...
export default function TaskDetailPage() {
  ...
  const saveJobDataImmediate = useCallback((jobId: string, chat: ChatMessage[], jobSteps: StepEvent[]) => {
  ...
  const loadJobChat = useCallback((job: typeof latestJob) => {
    ...
    // Load saved chat messages if available (dedupe to handle legacy data)
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (data.type === "job:started" && data.data?.liveUrl) {
          const url = data.data.liveUrl as string
          setLiveUrl(url)
        ...
        if (data.type === "job:step" || data.type === "job:progress") {
          ...
          if (stepData.goal || stepData.message) {
            const msgId = `step-${stepData.stepNumber ?? Date.now()}-${Math.random().toString(36).slice(2, 9)}`
            setChatMessages(prev => {
              // Avoid duplicate step messages
              if (stepData.stepNumber && prev.some(m => m.id.startsWith(`step-${stepData.stepNumber}-`))) {
                return prev
              }
              return [...prev, {
                id: msgId,
        ...
        if (data.type === "job:completed" || data.type === "job:failed") {
          ...
          // Use refs to get fresh data (avoids stale closure)
          ...
          // Save final state immediately (bypass debounce)
          if (data.jobId) {
            // Clear any pending debounced save
            if (saveTimeoutRef.current) {
              clearTimeout(saveTimeoutRef.current)
  ...
  useEffect(() => {
    return () => {
      ...
      // Flush save if there's an active job with data (use refs for latest values)
      const activeJobId = activeJobIdRef.current
      if (activeJobId && (chatMessagesRef.current.length > 1 || stepsRef.current.length > 0)) {
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setChatMessages(prev => [...prev, {
        id: `sys-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
        role: "assistant",
        content: "Message received. Note: Direct messaging to the agent is not yet supported. The agent follows its task instructions automatically. If you need to modify the task, please stop and restart with updated instructions.",
        timestamp: Date.now()
      }])
    }, 500)
  }
  ...
  if (isLoading) {
    return (
      <div className="overflow-hidden flex flex-col h-screen w-full" style={{ maxWidth: '100%', contain: 'inline-size' }}>
