...
import {
  ...
  FINAL_STEP_ID,
  JerseyGuidedSidebar,
} from "../../../../(portal)/org/[orgId]/returns/[returnId]/jersey-guided-finish/_components/jersey-guided-sidebar";
import { JerseyGuidedStepContent } from "../../../../(portal)/org/[orgId]/returns/[returnId]/jersey-guided-finish/_components/jersey-guided-step-content";

export default function ClientAccessJerseyGuidedFinishPage() {
  ...
  const formQuery = api.clientAccess.getJerseyCompanyReturnForm.useQuery(
    {
      token,
    },
    {
  ...
  const missingBySection = useMemo(() => {
    return {
      ...
      scheduleA: missingFields.filter((field) =>
        field.startsWith("scheduleA."),
      ),
  ...
  const initialStepId = useMemo(() => {
    ...
    for (const sectionId of sectionOrder) {
      const sectionMissing =
        missingBySection[sectionId as keyof typeof missingBySection] ?? [];
  ...
  const ensureFormExists = async () => {
    ...
    await createFormMutation.mutateAsync({ token });
  };
  ...
  const handleComplete = async () => {
    try {
      ...
      router.push(`/client-access/${token}/workspace`);
    } catch (error) {
      showMessage(
      ...
      <div className="flex h-[50vh] items-center justify-center">
        <div className="text-center">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            onClick={() => router.push(`/client-access/${token}`)}
          >
  ...
  if (
    ...
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <div className="text-center">
          <p className="text-sm text-muted-foreground">
            Guided finish for this access link is only available on Jersey company returns.
          </p>
          <Button
            className="mt-4"
            variant="outline"
